        ...
        def do_GET(self):
            ...
            if path == "/api/config":
                ...
                with state.lock:
                    ...
                    self.json(200 if job else 404, dict(job) if job else {"error": "Job not found."})
        ...
        def do_POST(self):
            ...
            if self.path.startswith("/api/shutdown"):
                ...
                if secrets.compare_digest(token, state.token) or (query_token and secrets.compare_digest(query_token, state.token)) or not token:
                    ...
                    self.json(200, {"status": "shutting_down"})
...
def start_watchdog(server, state, idle_timeout=10, initial_grace=25):
    def watchdog():
        while not state.shutdown_requested:
            ...
            if elapsed > idle_timeout:
                print("No active client heartbeat received. Shutting down server...", flush=True)
