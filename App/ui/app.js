...
async function pollJob(id, message, totalSteps) {
  ...
  for (;;) {
    ...
    if (job.stage === 'preparing' || backendPct === 0) {
      statusText = 'Preparing model & latents…';
    } else if (backendPct > 0) {
      statusText = `Denoising step ${currentStep} of ${jobTotalSteps}…`;
    }
...
$('composer').onsubmit = async event => {
  ...
  try {
    ...
    image.title = 'Click to open in default system photo viewer';
    ...
    const metadata = document.createElement('div'); metadata.className = 'metadata'; metadata.textContent = `${job.width} × ${job.height} · ${request.steps} steps · Seed ${job.seed}${request.transparent ? ' · Transparency requested' : ''}`;
